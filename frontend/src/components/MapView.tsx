...
export const MapView: React.FC<MapViewProps> = ({
  ...
  useEffect(() => {
    ...
    // Teselas CartoDB Voyager oficiales con API Key (elimina la marca de agua)
  ...
  useEffect(() => {
    ...
    if (radioMetros !== null && radioMetros > 0) {
      ...
      map.fitBounds(proximityCircleRef.current.getBounds(), {
        padding: [30, 30],
        maxZoom: radioMetros <= 500 ? 16 : radioMetros <= 1000 ? 15 : 14,
      });
    }
  }, [centro, radioMetros]);
  ...
  useEffect(() => {
    ...
    items.forEach((item: any) => {
      ...
      const style = CATEGORY_PINS[cat] || CATEGORY_PINS.cultura;
      ...
      marker.bindPopup(`
        <div class="flex flex-col gap-1.5 min-w-[180px]">
          <span class="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded w-fit text-white" style="background: ${style.bg};">
            ${style.icon} ${item.subcategoria || item.categoria}
          </span>
          <h4 class="font-bold text-xs md:text-sm text-slate-900 leading-tight">${titulo}</h4>
          ${item.direccion ? `<p class="text-[11px] text-slate-500">📍 ${item.direccion}</p>` : ''}
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Floating GPS and Reset Button Controls */}
      <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
        <button
          onClick={() => {
            onCenterChange({ lon: VITORIA_CENTER[1], lat: VITORIA_CENTER[0] });
            mapRef.current?.flyTo(VITORIA_CENTER, 14, { duration: 0.6 });
          }}
          className="bg-white/95 backdrop-blur-md p-2.5 rounded-xl shadow-md border border-slate-200 text-slate-700 hover:text-vitoria-forest hover:bg-slate-50 transition-all flex items-center gap-1.5 text-xs font-bold"
          title="Centrar en Virgen Blanca"
        >
          <span>🎯</span>
          <span className="hidden sm:inline">Virgen Blanca</span>
        </button>
        ...
        <button
          ...
          className="bg-white/95 backdrop-blur-md p-2.5 rounded-xl shadow-md border border-slate-200 text-slate-700 hover:text-emerald-700 hover:bg-slate-50 transition-all flex items-center gap-1.5 text-xs font-bold"
      ...
      <div className="absolute bottom-4 left-4 z-20 hidden md:flex items-center gap-2 bg-white/90 backdrop-blur-md py-1.5 px-3 rounded-full border border-slate-200 text-[11px] font-medium text-slate-600 shadow-sm">
        ...
        <span>Vitoria-Gasteiz • Bounding Box EPSG:4326</span>
      </div>
    </div>
  );
};
